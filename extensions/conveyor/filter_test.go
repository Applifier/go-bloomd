...
import (
	...
	"github.com/Applifier/go-bloomd/extensions/namer"
...
func readerReseter(keys ...string) input.KeyReaderReseter {
	var arr []bloomd.Key
	for _, key := range keys {
		arr = append(arr, bloomd.Key(key))
	}
	return input.NewArrayReaderReseter(arr...)
}
...
func createFilter(tb testing.TB, c *bloomd.Client, prefix string, period clock.UnitNum, unit clock.Unit) *Filter {
	tb.Helper()
	namer := namer.MustNewTimeUnitNamer(prefix, unit)
...
func generateSeqKeyReaderReseter(count int) input.KeyReaderReseter {
	var arr []bloomd.Key
	for i := 0; i < count; i++ {
